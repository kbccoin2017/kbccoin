...
#if defined(NDEBUG)
# error "Kbccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdb65aa217df7ea9004d65d42d1a1a57889e685349d8b394d34e44aaecc74cc8c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kbccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kbccoin Signed Message:\n";
...
{
    // Kbccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan =  12 * 60 * 60; // Kbccoin: 12 hours 
static const int64 nTargetSpacing = 5 * 60; // Kbccoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kbccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kbccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdb65aa217df7ea9004d65d42d1a1a57889e685349d8b394d34e44aaecc74cc8c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is KBCcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xda2dc61cfdf04791163802e867dba080d32553f21f60b4512bf4f6911405fe69"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kbccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KbccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KbccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KbccoinMiner(CWallet *pwallet)
{
    printf("KbccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kbccoin-miner");
    ...
    try { loop {
        ...
        printf("Running KbccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KbccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KbccoinMiner, pwallet));
}
